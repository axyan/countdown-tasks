...
describe('PUT /api/users/:userId Test Suite', () => {
  ...
  beforeEach(async () => {
    UPDATE_USER = await utils.createUserWithJWT(app, CURR_CRED.email, CURR_CRED.password);
  });
  ...
  test('should only update user email', async () => {
    ...
    const isMatch = await utils.comparePassword(NEW_CRED.newPassword, user.password);
...
describe('DELETE /api/users/:userId Test Suite', () => {
  ...
  beforeEach(async () => {
    DELETE_USER = await utils.createUserWithJWT(app, 'temp@email.com', 'password');
  });
